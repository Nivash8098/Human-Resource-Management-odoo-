...
export type AttendanceStatus = 'present' | 'absent' | 'half_day' | 'leave' | 'not_checked_in';
...
export type DocumentStatus = 'verified' | 'pending_verification' | 'expired';
...
export interface NotificationItem {
  ...
  title: string;
  message: string;
  type: 'leave_approved' | 'leave_rejected' | 'leave_submitted' | 'payroll_ready' | 'attendance_alert' | 'profile_update' | 'general';
  is_read: boolean;
  created_at: string;
  link?: string;
}
...
export interface ActivityItem {
  ...
  details: string;
  timestamp: string;
  type: 'check_in' | 'check_out' | 'leave_submitted' | 'leave_approved' | 'leave_rejected' | 'profile_updated' | 'payroll_generated' | 'document_uploaded';
}
